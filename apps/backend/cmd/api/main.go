...
func setupJobQueue(pool *pgxpool.Pool, auditRepo domainrepo.AuditRepository) (*handler.QueueHandler, *worker.Worker) {
	...
	// Register placeholder handlers for job types without specific implementations
	...
	// Register extract_lab_results handler with real use case
	...
	extractLabUC := usecase.NewExtractLabResultsUseCase(artifactRepo, measurementRepo, fileReader, auditRepo)
	w.RegisterHandler(entities.JobTypeExtractLabResults, handler.NewLabExtractJobHandler(extractLabUC))
