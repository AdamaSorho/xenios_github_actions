...
func setupJobQueue(pool *pgxpool.Pool, auditRepo domainrepo.AuditRepository) (*handler.QueueHandler, *worker.Worker) {
	...
	generateInsightsUC := usecase.NewGenerateInsightsUseCase(insightRepo, measurementRepo, auditRepo)
	insightHandler := handler.NewInsightGenerationHandler(generateInsightsUC)
	w.RegisterHandler(entities.JobTypeInsightGeneration, insightHandler.Handle)
