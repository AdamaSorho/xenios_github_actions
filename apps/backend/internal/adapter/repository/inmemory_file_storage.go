...
// Download retrieves the raw content of a stored object.
func (s *InMemoryFileStorage) Download(_ context.Context, key string) ([]byte, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
// PutObjectWithContent simulates adding an object with content to storage (for testing).
func (s *InMemoryFileStorage) PutObjectWithContent(key string, data []byte) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.objects[key] = true
